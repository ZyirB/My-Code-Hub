...
class Game(tk.Frame):
    def __init__(self, master):
        ...
        self.canvas.bind('<Left>', lambda _: self.paddle.move(-30)) # 10 original value
    ...
    def draw_text(self, x, y, text, size=24):
        return self.canvas.create_text(x, y, text=text, font=('Arial', size))
    
    def update_lives_text(self):
        if self.hud:
            self.canvas.delete(self.hud)
        self.hud = self.draw_text(50, 20, f'Lives: {self.lives}', size=14)
...
class Ball(GameObject):
    def __init__(self, canvas, x, y):
        self.radius = 10
        self.direction = [1, -1] # x nd y direction
        self.speed = 10
